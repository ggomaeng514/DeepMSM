...
if not main_args.wandb_off:
    ...
    if main_args.gbm_only:
        wandb.init(project="mri_",
                name=f"image_DINO_downstream_{main_args.seed}_GBM_only",
                notes=f"{now.strftime('%Y-%m-%d_%H:%M:%S')}",
                tags = ["nonTCGA2korean","gbm_only"]
                )
    else:
        wandb.init(project="mri_",
                name=f"image_DINO_downstream_{main_args.seed}",
                notes=f"{now.strftime('%Y-%m-%d_%H:%M:%S')}",
                tags = ["nonTCGA2korean"]
                )
...
os.makedirs(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}',exist_ok=True)

np.save(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/train_ids.npy', train_ids)
np.save(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/valid_ids.npy', valid_ids)
np.save(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/test_ids.npy', test_ids)
...
for epoch in tqdm(range(1,main_args.epochs+1)):
    ...
    with torch.no_grad():
        ...
        T_tr = T_tr.cpu()
        E_tr = E_tr.cpu()
        for i, (_, _,valid_data, t_mb, k_mb, m1_mb, m2_mb, m3_mb) in enumerate(valid_dataloader):
            X_val = valid_data.to(device)
            t_mb = t_mb.to(device)
            k_mb = k_mb.to(device)
            m1_mb = m1_mb.to(device)
            m2_mb = m2_mb.to(device)
        ...
        os.makedirs(f'./valid_result/dino/{cohort}/',exist_ok=True)
        f = open(f'./valid_result/dino/{cohort}/' + main_args.net_architect + f"_{now.strftime('%Y-%m-%d_%H:%M:%S')}_downstream.txt",'a')
        ...
        if epoch == 1:
            ...
            if threshold < C_Index.mean():
                ...
                threshold = C_Index.mean()
                print(epoch)
    os.makedirs(f'./model/dino/{cohort}/',exist_ok=True)
    torch.save(best_model, f'./model/dino/{cohort}/'+main_args.net_architect+f"_{now.strftime('%Y-%m-%d_%H:%M:%S')}_downstream_checkpoint_{main_args.seed}.pth")


model_path = f'./model/dino/{cohort}/'+main_args.net_architect+f"_{now.strftime('%Y-%m-%d_%H:%M:%S')}_downstream_checkpoint_{main_args.seed}.pth"
...
with torch.no_grad():
    ...
    T_tr_ = pd.DataFrame(T_tr, columns=['duration_'+main_args.spec_event])
    test_predictions['tr_y_structured'] = tr_y_structured
    test_predictions['T_tr_'] = T_tr_
    ...
    os.makedirs(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}',exist_ok=True)
    f = open(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/internal_downstream_seed_{main_args.seed}.txt', 'a')
    ...
    # 예측값을 파일로 저장  
    np.save(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/internal_test_predictions.npy', test_predictions)
...
with torch.no_grad():
    ...
    T_tr = T_tr.cpu()
    E_tr = E_tr.cpu()
    for i, (ids,_,Test_data, t_mb, k_mb, m1_mb, m2_mb, m3_mb) in enumerate(test_dataloader_external):
        X_ts = Test_data.to(device)
        t_mb = t_mb.to(device)
        k_mb = k_mb.to(device)
        m1_mb = m1_mb.to(device)
    ...
    os.makedirs(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}',exist_ok=True)
    f = open(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/external_downstream_seed_{main_args.seed}.txt', 'a')
    f.write('epoch {}'.format(ee) + '\n')
    ...
    np.save(f'./Test_result/dino/{cohort}/{main_args.net_architect}_{main_args.seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/external_test_predictions.npy', external_test_predictions)
