...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--dataset_name', type=str, default='SNUH_severance_UCSF') # 'TCGA' # 
...
if not main_args.wandb_off:
    ...
    if main_args.gbm_only:
        wandb.init(project="mri_",
                name=f"tabular_external_few_downstream_{main_args.seed}_GBM_only",
                notes=f"{now.strftime('%Y-%m-%d_%H:%M:%S')}",
                tags = ["nonTCGA2korean","gbm_only", "external_few_shot"]
                )
    else:
        wandb.init(project="mri_",
                name=f"tabular_external_few_downstream_{main_args.seed}",
                notes=f"{now.strftime('%Y-%m-%d_%H:%M:%S')}",
...
print(device)
PATH = './code/check/tabular/external_few_shot/'
...
os.makedirs(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/',exist_ok=True)

# Save IDs
np.save(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/train_ids.npy', train_ids)
np.save(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/test_ids.npy', test_ids)
...
model_path = "./code/load_trained_model/model/tabular/SNUH_severance_UCSF/Tabular_65_2025-02-25_17:13:31_downstream_checkpoint.pth" # internal model path
...
for epoch in tqdm(range(0,main_args.epochs)):
    ...
    with torch.no_grad():
        ...
        for i, (ids, test_tabular, _, t_mb, k_mb, m1_mb, m2_mb, m3_mb) in enumerate(test_dataloader):
        ...
        os.makedirs(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}',exist_ok=True)
        f = open(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/external_downstream_seed_{main_args.seed}.txt', 'a')
        ...
        np.save(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/external_test_predictions_epoch_{epoch}.npy', external_test_predictions)
        
        os.makedirs(f'./model/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}',exist_ok=True)
        torch.save(model, f'./model/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/'+main_args.net_architect+f"_epoch_{epoch}_{now.strftime('%Y-%m-%d_%H:%M:%S')}_downstream_checkpoint.pth")
        ...
        if threshold < mean_cindex:
            ...
            torch.save(model, f'./model/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/'+main_args.net_architect+f"_best_model_{now.strftime('%Y-%m-%d_%H:%M:%S')}_downstream_checkpoint.pth")
            np.save(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/external_test_predictions_best_model.npy', external_test_predictions)
            pd.DataFrame([external_results_table.iloc[epoch,:]], columns=external_results_table.columns).to_csv(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/' + main_args.net_architect+f"_results_best_epoch_{now.strftime('%Y-%m-%d_%H:%M:%S')}.csv", index=False)
external_results_table.to_csv(f'./Test_result/tabular/{cohort}/external_few_shot/few_{main_args.few_shot_num}/{main_args.net_architect}_{main_args.seed}_ex_seed_{main_args.external_seed}_{now.strftime("%Y-%m-%d_%H:%M:%S")}/' + main_args.net_architect+f"_results_table_{now.strftime('%Y-%m-%d_%H:%M:%S')}.csv", index=False)
